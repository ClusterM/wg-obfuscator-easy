...
def update_config():
    ...
    try:
        ...
        if "enabled" in data:
            ...
            config_manager.set("enabled", enabled_value, save=False)
            updated = True
        
        # Validate and update obfuscation_key
        if "obfuscation_key" in data:
            obfuscation_key = data["obfuscation_key"]
            if not isinstance(obfuscation_key, str):
                return jsonify({"error": "obfuscation_key must be a string"}), 400
            
            # Validate length (up to 300 characters)
